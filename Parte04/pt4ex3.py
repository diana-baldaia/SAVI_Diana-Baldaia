...
def main():
    ...
    index_params = dict(algorithm=1, trees=15)  # Using KDTree algorithm
    search_params = dict(checks=50)  # or pass empty dictionary
    flann_matcher = cv2.FlannBasedMatcher(index_params, search_params)  # Create the Flann Matcher object
    best_matches = flann_matcher.knnMatch(q_descriptors, t_descriptors, k=1) # Find the 2 best matches for each descriptor
