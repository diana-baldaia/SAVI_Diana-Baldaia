...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog='Image Stitcher',
        description='Stitches two images together',
        epilog='This is finished')


    parser.add_argument('-qi', '--query_image', type=str, default='Parte04/images/machu_pichu/2.png')
    parser.add_argument('-ti', '--target_image', type=str, default='Parte04/images/machu_pichu/1.png')
    ...
    Max_with = 800
    if q_image.shape[1]>Max_with:
        scale = Max_with/q_image.shape[1]
        q_image = cv2.resize(q_image, (0,0), fx=scale, fy=scale)
    if t_image.shape[1]>Max_with:
        scale = Max_with/t_image.shape[1]
        t_image = cv2.resize(t_image, (0,0), fx=scale, fy=scale)
