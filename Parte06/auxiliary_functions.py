...
def computeMosaic(q_image, t_image, mask):
...
def objectiveFunction(params, shared_mem):
    ...
    q_image_out_float = q_image_out.astype(float)
    target_image = shared_mem['t_image']
    target_image_float = target_image.astype(float)
    q_mask = shared_mem['q_mask']

    # Compute the error
    diff = np.abs(q_image_out_float - target_image_float)

    if shared_mem['q_mask'] is not None:
        mask_float = shared_mem['q_mask'].astype(float) / 255.0
        diff_mask = diff * mask_float
        error = np.sum(diff_mask) / np.sum(mask_float > 0)
    else:
        error = np.mean(diff)

    q_image_changed = changeImageColor(q_image, s=s, b=b, mask=q_mask)

    # Recompute the mosaic and show it
    mosaic_image = computeMosaic(q_image_out, target_image, q_mask)
